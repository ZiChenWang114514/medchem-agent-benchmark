...
const copy = {
  zh: {
    nav: ['数据概览', '能力覆盖', '完整样例', '合作方案'], eyebrow: 'MEDICINAL CHEMISTRY · AI TRAINING & EVALUATION',
  ...
  en: {
    nav: ['Dataset', 'Coverage', 'Full samples', 'Licensing'], eyebrow: 'MEDICINAL CHEMISTRY · AI TRAINING & EVALUATION',
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="section samples" id="samples"><div className="section-lead"><p className="section-number">03 · FULL SAMPLES</p><h2>{t.sampleTitle}</h2><p>{t.sampleText}</p><p className="sample-hint"><BookOpenCheck size={16} />{t.sampleNote}</p></div><div className="sample-stack">{sets.map((set) => <SampleSet key={set.set_id} set={set} language={language} />)}</div></section>
    <section className="section soft" id="licensing"><div className="section-lead compact"><p className="section-number">04 · LICENSING</p><h2>{t.cooperationTitle}</h2><p>{t.cooperationText}</p></div><div className="pricing-list">{plans.map(([name, description, price], index) => <div className={`price-row ${index === 2 ? 'recommended' : ''}`} key={name}><span className="plan-number">0{index + 1}</span><div><strong>{name}</strong><p>{description}</p></div><b>{price}</b></div>)}</div></section>
